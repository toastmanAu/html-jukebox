...
Publisher input/witness ordering concepts come from the owner’s `ckbfs-browser` repository. Its MIT notice is preserved in `licenses/ckbfs-browser.MIT.txt`. The legacy resolver and V2 codecs were not copied. The viewer informed future Blob lifecycle work; that player phase is not implemented yet.
