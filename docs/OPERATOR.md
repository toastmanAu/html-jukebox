...
Three.js demos may use a preceding inline import map that maps `three` to an absolute HTTPS module URL and `three/addons/` to an absolute HTTPS directory ending in `/`. Enable the network capability, review dynamic resource expressions, then verify the production preview. A bare module name without a matching map still blocks publication. The scanner checks mapped URL portability; the preview is required to catch unavailable CDN versions, CORS errors, or runtime failures.

## Demo screenshots

For a new demo, select an optional PNG, JPEG or WebP screenshot before saving the inspected publish draft. Files up to 20 MiB / 40 million decoded pixels are supported. The browser preserves aspect ratio, limits the longest edge to 960 pixels, and adjusts WebP quality (JPEG fallback) and dimensions until the result is at most 48 KiB. Processing happens locally. Review the resulting image and size before saving. The screenshot adds one V3 publication/signature before the usual manifest and registry transactions. Final serialized transaction size is still checked with wallet overhead included.

For an existing demo, choose **Load published demos for screenshot update**, select the demo, select/review a screenshot, and choose **Save screenshot update**. Continue and sign the screenshot, independently verify it, then publish/verify the new manifest and sign/verify the registry update. This takes three transactions and reuses the existing HTML, including its play-count identity. Replacing a screenshot follows the same flow; old images remain in history. If a draft is selected, choose **New draft** first. Screenshots cannot be changed after saving a draft; a signed image is retained for recovery.

Demos without screenshots retain the original cabinet artwork. Verified screenshots are lazy-loaded and cached in the existing IndexedDB store; checksum/hash/MIME failures remain visible. No screenshot transaction has been signed automatically by the coding agent: first live screenshot publication requires the owner JoyID through this interface.
