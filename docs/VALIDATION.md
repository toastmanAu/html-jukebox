...
The production build is now deployed at https://html-jukebox.pages.dev (initial deployment https://80ebe5a3.html-jukebox.pages.dev). All eight hosted desktop/mobile browser tests passed, including real on-chain catalog playback, cached replay, sandbox isolation, locked service access, and JoyID login UI. Custom domain htmljukebox.online is now active in Cloudflare with valid HTTPS. Actual owner JoyID login/signing on the public origin still requires the wallet holder.
...
Cloudflare reports both the htmljukebox.online zone and Pages custom domain active. Authoritative DNS returns Cloudflare addresses; HTTPS returns the app with the intended host CSP. The local recursive resolver still cached the old registrar address during verification, so the custom-domain browser suite used a temporary Chromium host-resolution override to the authoritative Cloudflare address, with normal TLS certificate validation enabled. This does not claim every recursive resolver has refreshed.

All eight custom-domain desktop/mobile acceptance tests passed: real Orbit Study catalog playback, verified cached replay and offline catalog, sandbox isolation, locked service deep links, and JoyID login UI. Wallet-holder authentication itself was not automated.
...
## Confirmed append head before byte comparison

Receipts 2 and 3 for astra-prisoner.html independently resolve to 163,840 and 245,760 cumulative bytes, respectively, under the original Type ID. Their witness chains and cumulative checksums verify; receipt 3 preserves the previously resolved first two prefixes exactly. Evidence: `evidence/pudge-astra-prisoner-append-1.json` and `evidence/pudge-astra-prisoner-append-2.json`. Full equality with the selected source still requires the persisted browser draft.

Fixed a timing path where a pending append left its predecessor live: resolving that shorter prefix could incorrectly report CONTENT_HASH_MISMATCH. Verification now checks the saved transaction's commitment first, then requires discovery to return that exact transaction head. Stale discovery reports INDEXER_SYNC_PENDING when the expected output remains live; a spent expected output with a different discovered head reports CELL_CHANGED. Integrity errors still block progression. All 118 unit/integration tests and the production build pass, including pending, stale, superseding, and exact-head cases.

## Live automatic large-file acceptance — passed

Independent Pudge verification on 2026-09-12 confirms the complete astra-prisoner.html file: 711,510 bytes across nine transactions under the original Type ID, with valid cumulative checksums and content hash `0xf2ae80e7b8e92e68d25c5094c8df4379acd8c5d9ea418d3c935f478ac0217c4c`, matching the published manifest. Receipt 10 (`0xc8930ce588d8937b7f1c5d6a83fe0297369d56077eb49ee13bc045ecc3a6eba0`) published manifest revision 5; registry update `0x41e3b382fca299be6582440e6c49089425468f17a30630caa4dc437cab5544fc` activated it. The wallet holder reports successful playback.

The operator subsequently published glm53f-cryptoAnalyser.html: 149,517 bytes across two transactions, also independently checksum/hash verified. Registry revision 6 at `0xf7209e71bcca7ac24a594bf23025fa4e2edaacdbafb05516eca360c93920db56` contains Orbit Study and both new demos. Both registry transitions consume the prior registry, preserve its Type ID and owner lock, and reference hash-verified manifests with the correct previous pointers. All three current demo byte lengths and content hashes match the catalog. Evidence: `evidence/pudge-catalog-revision-6.json`. Source bytes are not included in the evidence. This completes live acceptance of the automatic segmented upload workflow; the new crypto analyser's interactive behavior was not independently browser-tested in this checkpoint.

## Shared play tracker — deployed 2026-09-13

Deployment `6cb40bb6.html-jukebox.pages.dev` adds shared per-demo load counts on the cards and selected details. The production Pages Function uses the `PLAYS` D1 binding, with migration `0001_plays.sql` applied. Each sandbox load is counted once by an anonymous load UUID; host reloads create another load, and admin previews are excluded. SQL trigger/receipt tests verify atomic increments and duplicate suppression, including retry after a lost response. Counter failure leaves playback available. Counts begin at deployment and do not reconstruct historical usage.

All 122 unit/integration tests, the production build, and the Pages Functions bundle build passed. Desktop/mobile browser tests verify one count per load and reload, plus sandbox isolation. All ten hosted browser checks passed on htmljukebox.online, including real Orbit Study playback and separate fresh visitors observing the shared count. The hosted checks replay an actual load receipt; exact duplicate suppression is asserted independently by the SQLite integration test. Browser acceptance loads contribute real plays.
...
Screenshot release `652bddfd.html-jukebox.pages.dev` is deployed on htmljukebox.online. All ten hosted desktop/mobile checks passed, including the verified offline thumbnail fixture, sandbox playback, live Pudge catalog, and shared play counters. The two source-module compression checks are intentionally skipped on the hosted build and passed separately against local Vite. Owner-signed screenshot publication remains the final live chain acceptance step.
