...
export function AdminPanel() {
  ...
  return <section className="panel full" aria-label="Publish a demo"><p className="eyebrow">SERVICE PANEL / IMMUTABLE DEMO PUBLISH</p><h2>Put a new demo on the jukebox.</h2>
    ...
    {current && <div className="estimate"><h3>{current.local.metadata.title} · {current.stage}</h3><p>{current.local.bytes.toLocaleString()} bytes · <code>{current.local.contentHash}</code></p>{current.error && <p className="error">{current.error}</p>}
      ...
      {current.previewed && current.stage !== 'COMPLETE' && <div className="actions"><button disabled={busy || !signer || !!pending} onClick={() => action(async () => { invariant(signer, 'WALLET_REQUIRED', 'Connect JoyID.'); const result = await advanceDraft(current.draftId, signer); setPending(result.action ? { action: result.action, signer } : undefined); setStatus(result.action ? `Review the ${result.action.kind} transaction before signing.` : `Stage verified: ${result.draft.stage}. Continue to the next safe step.`); })}>Continue next safe step</button><button disabled={busy || !signer || !current.demo} onClick={() => action(async () => { invariant(signer, 'WALLET_REQUIRED', 'Connect JoyID.'); await recoverDraftTransaction(current.draftId, signer); setPending(undefined); setStatus('Known transaction re-queried. Continue after it commits.'); })}>Recover known transaction</button></div>}
      {[current.demo, current.manifest?.receipt, current.registry].filter(Boolean).map(receipt => <p key={receipt!.txHash}><a href={`https://pudge.explorer.nervos.org/transaction/${receipt!.txHash}`} target="_blank" rel="noreferrer">{receipt!.txHash} ↗</a></p>)}
      {pending && <div><h3>{pending.action.kind} · TESTNET/PUDGE</h3><p>Locked capacity {ckb.fixedPointToString(BigInt(pending.action.prepared.estimate.capacity))} CKB · fee {ckb.fixedPointToString(BigInt(pending.action.prepared.estimate.fee))} CKB · {pending.action.prepared.estimate.transactionBytes} serialized bytes</p><button disabled={busy || signer !== pending.signer} onClick={() => action(async () => { invariant(signer === pending.signer, 'WALLET_CHANGED', 'Reconnect the same wallet.'); await signDraftAction(pending.action, signer); setPending(undefined); setStatus('Transaction broadcast and persisted. Continue to independently verify after commitment.'); })}>Sign {pending.action.kind} transaction on Pudge</button></div>}
    </div>}
  </section>;
}
