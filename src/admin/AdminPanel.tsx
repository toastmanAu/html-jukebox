...
export function AdminPanel() {
  ...
  async function saveNewDraft() {
    invariant(file && signer, 'OWNER_REQUIRED', 'Select a file and connect the owner JoyID first.');
    ...
    invariant(!report.externalUrls.length || caps.network, 'NETWORK_CAPABILITY_REQUIRED', 'Enable network access for this file’s external HTTPS resources.');
    const latestRegistry = await loadOwner();
    const draft = await createDraft(file.filename, file.bytes, { id: crypto.randomUUID(), title: title.trim(), description, category: category.trim(), tags: tags.split(',').map(t => t.trim()).filter(Boolean), capabilities: caps }, latestRegistry);
  ...
  const saveBlockers = [
    ...
    !file && 'Select a single-file HTML demo.',
    !title.trim() && 'Enter a demo title.',
    !category.trim() && 'Enter a category.',
    !!file && !report?.acceptable && 'Fix the portability errors listed above; relative resources must be embedded or use absolute HTTPS URLs.',
    !!report?.issues.some(issue => issue.kind === 'dynamic') && !dynamicReviewed && 'Review the dynamic resource references and tick the review checkbox above.',
    !!report?.externalUrls.length && !caps.network && 'Enable network under Approved capabilities for the external HTTPS resources.',
  ...
  return <section className="panel full" aria-label="Publish a demo"><p className="eyebrow">SERVICE PANEL / IMMUTABLE DEMO PUBLISH</p><h2>Put a new demo on the jukebox.</h2>
    ...
    <div className="actions"><button disabled={busy || !previewBytes} onClick={() => { setPreview(true); setPreviewSeen(false); }}>Open production-sandbox preview</button><button disabled={saveBlockers.length > 0} aria-describedby="draft-save-help" onClick={() => action(saveNewDraft)}>Save inspected publish draft</button></div>
